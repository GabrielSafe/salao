...
import {
  ...
  Plus, Check, X, Loader2, User, ShieldCheck, ChevronDown, UserCheck
} from 'lucide-react';
...
function AtribuirBtn({ atendimentoId, tipoServico, funcionarias }) {
  ...
  function handleOpen() {
    ...
    if (!rect) return;
    const estimatedH = Math.min(candidatas.length * 46 + 70, 280);
    ...
    <div style={{ position: 'relative' }}>
      ...
      {aberto && createPortal(
        <>
          ...
          <div style={{ position: 'fixed', top: dropPos.top, right: dropPos.right, background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 10, zIndex: 9999, overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,.15)', minWidth: 190 }}>
            ...
            {candidatas.length === 0
              ...
              : candidatas.map(f => (
                <button key={f.id} onClick={() => atribuir(f.id)} disabled={!!loading}
                  style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'none', border: 'none', cursor: 'pointer', borderBottom: '1px solid rgba(0,0,0,.04)' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(96,165,250,.05)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'none'}
                >
                  <div style={{ width: 26, height: 26, borderRadius: '50%', background: `${SC[f.status]}20`, border: `1.5px solid ${SC[f.status]}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: SC[f.status], flexShrink: 0 }}>
            ...
            <button onClick={() => setAberto(false)} style={{ width: '100%', padding: '7px', background: 'rgba(0,0,0,.02)', border: 'none', cursor: 'pointer', fontSize: 11, color: '#9CA3AF' }}>
              Cancelar
            </button>
          </div>
        </>,
